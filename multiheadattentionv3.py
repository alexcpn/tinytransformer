...
class MultiHeadSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        Q = self.W_Q(x).reshape(B, seq_len, self.num_heads, self.head_dim).permute(
            0, 2, 1, 3)  # (B, num_heads, seq_len, head_dim)
        K = self.W_K(x).reshape(B, seq_len, self.num_heads,
                                self.head_dim).permute(0, 2, 1, 3)
        V = self.W_V(x).reshape(B, seq_len, self.num_heads,
                                self.head_dim).permute(0, 2, 1, 3)
        ...
        causal_mask = torch.triu(torch.ones(
            seq_len, seq_len, device=x.device), diagonal=1).bool()
        ...
        attention_output = attention_output.permute(
            0, 2, 1, 3)  # (B, seq_len, num_heads, head_dim)
        ...
        attention_output = torch.mean(
            attention_output, dim=2)  # (B, seq_len, head_dim)
